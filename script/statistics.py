...
df = pd.read_csv('../data/spectral_result.csv')
...
for i in range(-1, 4):
    ...
    result_li.append([i, len(cluster), avg_temp_q1, avg_temp_q2, avg_temp_q3, avg_temp_q4, avg_rain_q1, avg_rain_q2, avg_rain_q3, avg_rain_q4])
# write to csv
result = pd.DataFrame(result_li, columns=['cluster', 'count', 'avg_temp_q1', 'avg_temp_q2', 'avg_temp_q3', 'avg_temp_q4', 'avg_rain_q1', 'avg_rain_q2', 'avg_rain_q3', 'avg_rain_q4'])
print(result)
result.to_csv('../data/spectral_statistics.csv', index=False)